...
4. Design a reward function that penalizes both PnL variance and transaction costs
...
│   └── results/
│       ├── delta_hedge_pnl.png     ← PnL distribution of delta hedge
...
$$r_t = -\text{PnL variance} - \lambda \cdot |\Delta \text{position}| \cdot \text{transaction cost}$$

Where $\lambda$ controls the transaction cost penalty.

### Step 4 — Evaluation
Compare RL vs delta hedge on:
- PnL distribution (mean, std, VaR)
...
**Reward function**: $r_t = -\text{PnL}_t^2 - \lambda \cdot |\Delta\delta| \cdot S_t$

Penalizes PnL variance (squared) and transaction costs simultaneously — consistent with the Deep Hedging paper's convex risk measure framework.
...
| Total PnL (random) | -6.27 |

> Negative PnL with random actions is expected — the RL agent will learn to do better.
...
|---|---|---|
| Mean PnL | -0.1358 | Slightly negative due to transaction costs |
| Std PnL | 2.6437 | Residual risk from discrete rebalancing |
...
> The slightly negative mean PnL is entirely explained by transaction costs — in a frictionless market with continuous rebalancing, delta hedge PnL would be exactly 0. The residual std of 2.64 comes from discrete rebalancing (50 steps instead of continuous).
...
> The reward is always negative by design ($r = -\text{PnL}^2 - \text{tc}$). A perfect hedge would give reward = 0. The agent improved from -164 to -36 — a 78% improvement.
...
|---|---|---|---|
| Mean PnL | -0.2118 | **+0.3823** | RL ↑ |
| Std PnL | 2.6104 | 7.0613 | DH ↑ |
...
The RL agent achieves a positive mean PnL (+0.38) and positive Sharpe (+0.054) — it learned to extract value from the market. However, its tail risk is dramatically worse: CVaR of -15.59 vs -6.27 for the delta hedge.

**Why this happens — the key lesson:**

The agent optimized $r = -\text{PnL}^2 - \text{tc}$ and found an unintended strategy: take aggressive positions that generate positive average PnL while accepting large occasional losses. The reward penalizes step-level variance but does not control tail risk (CVaR).
...
- V1: $r_t = -\text{PnL}_t^2 - \text{tc}_t$ at every step
- V2: $r_t = -\text{tc}_t$ at intermediate steps, $r_T = \text{PnL}_{total} - \lambda \cdot \max(0, -\text{PnL}_{total})^2$ at terminal step

**Full 3-way comparison (1000 out-of-sample episodes):**

| Metric | Delta Hedge | RL v1 (variance) | RL v2 (CVaR) |
|---|---|---|---|
| Mean PnL | -0.2118 | +0.3823 | **+2.6685** |
| Std PnL | 2.6104 | 7.0613 | 11.6753 |
...
The terminal CVaR penalty is given at step 50 only. PPO cannot connect this terminal signal back to each of the 50 individual hedging decisions. The agent ignores the tail risk penalty and maximizes mean PnL instead.
...
| Strategy | Mean PnL | Sharpe | CVaR 95% | Verdict |
|---|---|---|---|---|
...
- Standard RL (PPO) beats delta hedge on mean PnL and Sharpe but worsens tail risk
...
> An RL agent that explicitly optimizes for PnL variance and transaction costs will outperform a textbook delta hedge in realistic market conditions — especially when transaction costs are non-negligible or volatility is misspecified.
