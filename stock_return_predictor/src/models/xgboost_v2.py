...
model = XGBClassifier(
    ...
    learning_rate=0.05,
    max_depth=4,
    subsample=0.8,          # prevents overfitting by sampling 80% of rows per tree
...
# Use predicted probabilities instead of binary predictions
# Only trade when the model is confident enough
...
for threshold in [0.50, 0.55, 0.60]:
    ...
    n_trades = signal.sum()
    acc = accuracy_score(y_test[signal == 1], (y_proba[signal == 1] > 0.5).astype(int)) if n_trades > 0 else 0
